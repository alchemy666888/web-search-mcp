...
Reopened: 2026-07-24 — The previous approval was invalidated after implementation evidence showed that the approved `pnpm@11.17.0` bootstrap requires a registry download blocked by the implementation environment. The replacement policy uses the preinstalled `pnpm@10.28.1` and makes an approved dependency source an explicit installation prerequisite.
...
`package.json` pins exact versions, declares `packageManager: "pnpm@10.28.1"`, and `pnpm-lock.yaml` is committed. Runtime dependencies are `next@16.2.11`, `react@19.2.8`, `react-dom@19.2.8`, `@modelcontextprotocol/sdk@1.29.0`, and `zod@4.4.3`. Development tooling is pinned to Node.js 24, pnpm 10.28.1, TypeScript 5.9.3, Vitest and `@vitest/coverage-v8` 4.1.10, ESLint 9.39.5, and `eslint-config-next@16.2.11`, with matching stable type packages.
