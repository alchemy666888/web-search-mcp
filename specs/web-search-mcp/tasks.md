...
- `.npmrc` — enforce exact dependency saves.

Steps:

1. Create the minimal Next.js/TypeScript project manifests without adding a UI, database, cache, analytics package, or unapproved runtime dependency.
2. Pin the approved versions: Next.js 16.2.11, React/React DOM 19.2.8, MCP SDK 1.29.0, Zod 4.4.3, pnpm 11.17.0, TypeScript 5.9.3, Vitest/coverage 4.1.10, ESLint 9.39.5, and `eslint-config-next` 16.2.11, plus matching stable type packages.
3. Define `lint`, `typecheck`, `test`, `test:coverage`, `build`, and later-compatible `smoke` scripts.
4. Generate and commit the pnpm lockfile using Node.js 24 and the pinned package manager.
5. Configure strict compilation, no JavaScript emission for typecheck, test discovery under `tests/`, and the approved coverage thresholds.

Verification:

- `node --version` reports Node.js 24.x.
- `pnpm --version` reports 11.17.0.
...
- A clean install is reproducible from `package.json` and `pnpm-lock.yaml`.
...
8. Run the complete release gate from a clean dependency install.
...
1. Document Node/pnpm versions, exact install and quality commands, and the two required Vercel environment variables without sample secret values.
